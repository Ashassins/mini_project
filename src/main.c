...
int main(void) {
  ...
  Sprite bolts[5];
  for (int i = 0; i < 5; i++) {
    init_sprite(1000, 1000, lightning_a_width, lightning_b_width,
  ...
  while (!(all_dead)) {
    if ((glbcnt + 1) % 15 == 0) {
      // -----4FPS (BASICALLY FREE)-----
      //    	LCD_DrawString(230,300,0x0F00,0x0000, "Don't mind the spaz monke
      //    uwu", 16, 0);
      draw_score(140, 300);
      ...
      //    	invader.sprite_data =
      //            (uint16_t *)(((uint32_t)invader.sprite_data) ^
      //            invader.sprite_swap_key);
